...
sta_time = (datetime.datetime.now()+datetime.timedelta(hours=-12)).strftime("%Y-%m-%d %H:%M:%S")
for x in mycol.find({"operDate":{"$gte":sta_time,"$lt":end_time},"operation":{'$regex': '登录'}},{"address":"1","siteId":"1"}):
    str_original = x["address"].split()
    str_province = str_original[0]
    str_site = x["siteId"]
    if str_province == "中国":
        str_city = str_original[1]
        map_city(str_province,str_city,str_site)
    elif str_province == "未知地址":
        a = 1
    elif str_province == "保留地址":
        a = 2
    else:
        map_province(str_province,str_site)    
...
data_dist = {}
num = 0
for item in data_coor:
        s = str(item)
        if s in data_dist.keys():
            data_dist[s] = data_dist[s] + 1
        else:
            data_dist[s] = 1
for item in data_dist.keys():
    string = item.replace('\'', '').replace('[', '').replace(']', '').replace(' ', '')
    last_world = string.split(',')
    site = last_world[2]
    for value in Webmaster_value.keys():
        if str(site) == str(value):
            site = Webmaster_value[value]
    last_world[2] = data_dist[item]
    abc = [last_world]
    try:
        geolines.add(str(site),abc, tooltip_formatter="{a} : {c}",**style_geo_all)
...
for item in data_world:
        s = str(item)
        if s in dist_world.keys():
            dist_world[s] = dist_world[s] + 1
        else:
            dist_world[s] = 1

for item in dist_world.keys():
    string = item.replace('\'', '').replace('[', '').replace(']', '').replace(' ', '')
    last_world = string.split(',')
    site = last_world[2]
    for value in Webmaster_value.keys():
...
geolines.render('/opt/jumpserver/apps/templates/map_render.html')
os.system("sed -i \'s#\"roam\": true,#\"roam\": true,\\n\"center\": [100, 34],\\n\"zoom\": 3.5,#g\' /opt/jumpserver/apps/templates/map_render.html")

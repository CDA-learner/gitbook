...
########################################################
    def _on_new_message(self, msgs):
        print('_on_new_message',msgs)
        messages_content = msgs['data']['post']['message'].split('@')[1:]
        for item in messages_content:
            item = '@' + item
            msgs['data']['post']['message'] = item
            print('转换后msgs', msgs)
            if self.ignore(msgs) is True:
                return
            msg = self.filter_text(msgs)
            print('responsed to xxxx')
